...
var wheelConfig = {
    assetPath : 'pink/',        // relative path to asset folder
    ...
    createWheel: function(scene) {
        ...
        // add sprites
        scene.wheelborder = addSprite('wheelborder', 0, 65, depth);
        ...
        scene.lightsector = addSprite('lightsector', 0, 92, depth)?.setAlpha(0)?.setOrigin(0.5, 0.98);
        ...
        scene.pointer = addSprite('pointer', 0, -240, depth).setOrigin(0.5, 0.43);
        scene.centerpin = addSprite('centerpin', 0, 125, depth);
    ...
    createLamps: function(scene){
        var lCount = 9; var lOffset = 335; var dY = 93; 
        scene.lampsArray = []; 
