...
var wheelConfig = {
    ...
    sprites: [
        {
            fileName: 'xmas_back.png',   // Christmas background image
            name: 'background',
        },
        {
    ...
    createWheel: function(scene) {
        let depth = 0;
        console.log('Christmas createWheel started, theme:', window.GAME_CONFIG && window.GAME_CONFIG.theme);
        ...
        console.log('Checking if background texture exists:', scene.textures.exists('background'));
        if (scene.textures.exists('background')) {
            ...
            console.log('Christmas background created! Size: 2400x1400, Depth: -100, Position:', scene.centerX, scene.centerY);
        } else {
            console.error('Christmas background texture not found!');
        }
