...
var wheelConfig = {
    assetPath : 'antique/',        // relative path to asset folder
    centerOffsetX: 0,           // wheel offset from canvas center X
    centerOffsetY: -190,         // wheel offset from canvas center Y
    ...
    createWheel: function(scene) {
        ...
        scene.wCont = scene.add.container(scene.centerX, scene.centerY + 60); 

        scene.wheel = addSprite("wheel", 0 - scene.wCont.x, 60 - scene.wCont.y, depth)?.setOrigin(0.5, 0.5).setAngle(11.25);
        ...
        scene.pointer = addSprite('pointer', 0, -240, depth).setOrigin(0.5, 0);
        scene.centerpin = addSprite('centerpin', 0, 85, depth);
        scene.pointerBorder = addSprite('pointerborder', 0, -245, depth);
