...
var wheelConfig = {
    ...
    createWheel: function(scene) {
        ...
        scene.wCont = scene.add.container(scene.centerX, scene.centerY + 66); 

        scene.wheel = addSprite("wheel", 0 - scene.wCont.x, 66 - scene.wCont.y, depth)?.setOrigin(0.5, 0.5).setAngle(0);
        scene.lightsector = addSprite('lightsector', 0, 60, depth)?.setAlpha(0)?.setOrigin(0.5, 0.95);
        ...
        scene.pointer = addSprite('pointer', 0, -260, depth).setOrigin(0.5, 0.4);
        scene.centerpin = addSprite('centerpin', 0, 80, depth);
