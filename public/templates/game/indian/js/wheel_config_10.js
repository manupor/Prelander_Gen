...
var wheelConfig = {
    ...
    createWheel: function(scene) {
        ...
        scene.wheelborder = addSprite('wheelborder', 0, 78, depth);

        scene.wCont = scene.add.container(scene.centerX, scene.centerY - 15); 

        scene.wheel = addSprite("wheel", 0 - scene.wCont.x, -15 - scene.wCont.y, depth)?.setOrigin(0.5, 0.5).setAngle(0);
        scene.lightsector = addSprite('lightsector', 0, -15, depth)?.setAlpha(0)?.setOrigin(0.5, 0.96);
        ...
        scene.pointer = addSprite('pointer', 0, -325, depth).setOrigin(0.5, 0.05);
        scene.centerpin = addSprite('centerpin', 0, 10, depth);
        scene.pointerBorder = addSprite('pointerborder', 0, -325, depth);

        // adding the text field
        scene.prizeText = scene.add.bitmapText(scene.centerX, scene.centerY + 535, 'gameFont_1', 'SPIN THE WHEEL', 36, 1).setOrigin(0.5);
